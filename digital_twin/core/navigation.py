    ...
    def wait(
        ...
        *,
        ...
        while self.should_continue() and time.monotonic() < deadline:
            ...
            self.cancel_event.wait(min(0.1, max(0.0, deadline - time.monotonic())))
    ...
    try:
        ...
        while route_index < len(routes) and ctx.should_continue():
            ...
            if route_delay > 60.0 and route_index < len(routes):
                ...
                try:
                    replanned = plan_mission(origin, remaining_stops, start_time=config.local_now())
                    ...
                    db.update_mission(
                        ...
                        planned_route_points=_point_count(plan),
        ...
        db.update_mission(
            ctx.mission_id,
            status="stopped" if not worker_shutdown.is_set() else "completed",
            worker_id=None,
            is_holding_final_position=0,
        )
    except Exception as exc:
...
def run_worker(poll_interval: float = 0.5) -> None:
    logger.init_server_logs("worker")
    db.init_db()
    ...
    def maintain_history() -> None:
        while not worker_shutdown.is_set():
            try:
                logger.archive_expired_sessions(30)
                logger.cleanup_archive_cache()
            except Exception as exc:
